...
- **Réconciliation Authentik** : purge ORM puis REST. **`repair-wendigo-pulumi-state.py`** avant refresh : supprime les entrées d'état `default_*` et les flows orphelins en API (sans `pulumi import`). Token via `AUTHENTIK_TOKEN`, puis **`pulumi refresh -y --parallel 2`**, puis `pulumi up`. Backend recréé après `pulumi up` pour JWKS.
