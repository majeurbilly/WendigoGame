...
- **Réconciliation Authentik** : purge ORM + repli REST ; rotation token + `sync_authentik_provider` avant `pulumi refresh` (évite 403 token expiré figé dans l'état provider).
