...
- **Réconciliation Authentik** : purge ORM puis REST. **`repair-wendigo-pulumi-state.py`** supprime état `default_*` + flows orphelins API, puis **`sync_authentik_provider`** (`pulumi up --target` token), **`pulumi refresh -y --parallel 2`**, `pulumi up`.
