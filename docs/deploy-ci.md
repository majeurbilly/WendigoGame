...
- **Réconciliation Authentik** : purge ORM puis REST. **`repair-wendigo-pulumi-state.py`** avant et **après** `sync_authentik_provider` (side-effects de `pulumi up --target`), puis **`pulumi refresh -y --parallel 2`**, `pulumi up`.
